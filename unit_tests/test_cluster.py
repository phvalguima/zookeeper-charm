...
class TestCluster(unittest.TestCase):
    ...
    @patch.object(cluster.ZookeeperCluster, "unit", new_callable=PropertyMock)
    @patch.object(cluster.ZookeeperCluster, "relation", new_callable=PropertyMock)
    def test_get_all_tls_certs(self,
        ...
        zk = harness.charm
        data = {
            "u1": { "tls_cert": certs[1]["crt"], "myid": 1, "endpoint": "ansiblezookeeper2.example.com:2888:3888" },
            "u2": { "tls_cert": certs[2]["crt"], "myid": 2, "endpoint": "ansiblezookeeper3.example.com:2888:3888" },
            "u3": { "tls_cert": certs[3]["crt"], "myid": 3, "endpoint": "ansiblezookeeper1.example.com:2888:3888" }
        }
        mock_relation.return_value = MockRelation(data)
        mock_unit.return_value = "u1"
        tspwd = genRandomPassword()
        zk.cluster.set_ssl_keypair(certs[1]["crt"], "/tmp/test-ts-quorum.jks", tspwd, "ubuntu", "ubuntu", 0o640)
        self.assertEqual(len(tspwd), 48)
        self.assertEqual(True, security._check_file_exists("/tmp/test-ts-quorum.jks"))
        __cleanup()
